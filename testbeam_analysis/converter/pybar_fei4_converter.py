...
def analyze_raw_data(input_file, trigger_data_format):  # FE-I4 raw data analysis
    ...
    # Set output file name if multiple input file names
    analyzed_data_file = None
    if isinstance(input_file, list):
        analyzed_data_file = input_file[0][:-3] + '_interpreted.h5'
...
def process_dut(raw_data_file, trigger_data_format=0, do_corrections=False):
    ...
    Parameters
    ----------
    raw_data_file : string, iterable
        file or files with raw data from one run
    ...
    if isinstance(raw_data_file, list):
        raw_data_file = raw_data_file[0]
    ret_value = align_events(raw_data_file[:-3] + '_interpreted.h5',
                             raw_data_file[:-3] + '_event_aligned.h5',
                             fix_trigger_number=fix_trigger_number,
                             fix_event_number=fix_event_number)
    format_hit_table(raw_data_file[:-3] + '_event_aligned.h5',
                     raw_data_file[:-3] + '_aligned.h5')
    return ret_value
...
def align_events(input_file, output_file, fix_event_number=True, fix_trigger_number=True, chunk_size=20000000):
    ...
    with tb.open_file(input_file, 'r') as in_file_h5:
        ...
        with tb.open_file(output_file, 'w') as out_file_h5:
            ...
            hit_table_out = out_file_h5.create_table(out_file_h5.root,
                                                     name='Hits',
